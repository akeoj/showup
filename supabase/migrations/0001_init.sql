...
as $$
  with base as (
    select
      c.*,
      (select count(*) from public.participants p where p.challenge_id = c.id) as participant_count,
      (select count(*) from public.workouts w
        where w.challenge_id = c.id
          and w.workout_date = (now() at time zone c.timezone)::date) as active_today,
      (select u.nickname from public.users u where u.id = c.creator_id) as creator_nickname
    from public.challenges c
    where c.visibility = 'public'
  )
  select
    id, name, code, activity_type, tracking_mode, unit, daily_target,
    start_date, end_date, description, participant_count, active_today, creator_nickname
  from base
  where case
    when p_filter = 'ending_soon' then end_date >= current_date
    when p_filter = 'new'         then true
    else true
  end
  order by
    case when p_filter = 'new'         then extract(epoch from created_at) end desc nulls last,
    case when p_filter = 'ending_soon' then -extract(epoch from end_date::timestamptz) end desc nulls last,
    case when p_filter = 'most_active' then active_today end desc nulls last,
    -- 'trending' default: recent participation weighted by recency
    (participant_count + active_today * 3) desc,
    created_at desc
...
begin
  ...
  with completed as (
    ...
    select w.user_id, w.workout_date
  ...
  runs as (
    select user_id, grp, count(*)::int as run_len, max(workout_date) as run_end
  ...
  streaks as (
    select
      ...
      coalesce(max(run_len) filter (where run_end >= v_today - 1), 0) as current
    from runs group by user_id
  ),
  totals as (
    select
      w.user_id,
      coalesce(sum(w.count), 0)::bigint as total_count,
      coalesce(sum(w.count) filter (where w.workout_date = v_today), 0)::int as today_count,
      count(*) filter (where w.count >= v_challenge.daily_target)::bigint as completed_days
    from public.workouts w
    where w.challenge_id = p_challenge_id
    group by w.user_id
  )
  select
    ...
    coalesce(t.completed_days, 0),
    coalesce(s.current, 0),
    coalesce(s.longest, 0)
  ...
  order by coalesce(t.total_count, 0) desc, coalesce(s.longest, 0) desc, p.joined_at asc;
end;
$$;
